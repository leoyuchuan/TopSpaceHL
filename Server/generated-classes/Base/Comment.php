...
abstract class Comment implements ActiveRecordInterface
{
    ...
    public function setNewsId($v)
    {
        ...
        if ($this->news_id !== $v) {
            $this->news_id = $v;
            $this->modifiedColumns[CommentTableMap::COL_NEWS_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : CommentTableMap::translateFieldName('NewsId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->news_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : CommentTableMap::translateFieldName('CommentId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->comment_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : CommentTableMap::translateFieldName('Content', TableMap::TYPE_PHPNAME, $indexType)];
            $this->content = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : CommentTableMap::translateFieldName('Date', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(CommentTableMap::COL_NEWS_ID)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(CommentTableMap::COL_NEWS_ID)) {
            $criteria->add(CommentTableMap::COL_NEWS_ID, $this->news_id);
        }
        if ($this->isColumnModified(CommentTableMap::COL_COMMENT_ID)) {
    ...
    {
        return (null === $this->getNewsId()) && (null === $this->getCommentId());
    }
    ...
    {
        $copyObj->setNewsId($this->getNewsId());
