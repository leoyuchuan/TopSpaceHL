...
class CommentTableMap extends TableMap
{
     ...
     */
    const COL_NEWS_ID = 'COMMENT.news_id';
    ...
    public function initialize()
    {
        ...
        // columns
        $this->addPrimaryKey('news_id', 'NewsId', 'INTEGER', true, 32, null);
    ...
    public static function addInstanceToPool($obj, $key = null)
    {
        if (Propel::isInstancePoolingEnabled()) {
            if (null === $key) {
                $key = serialize(array((string) $obj->getNewsId(), (string) $obj->getCommentId()));
            } // if key === null
    ...
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \Comment) {
                $key = serialize(array((string) $value->getNewsId(), (string) $value->getCommentId()));
    ...
    public static function getPrimaryKeyHashFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        if ($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('NewsId', TableMap::TYPE_PHPNAME, $indexType)] === null && $row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('CommentId', TableMap::TYPE_PHPNAME, $indexType)] === null) {
            return null;
        }

        return serialize(array((string) $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('NewsId', TableMap::TYPE_PHPNAME, $indexType)], (string) $row[TableMap::TYPE_NUM == $indexType ? 1 + $offset : static::translateFieldName('CommentId', TableMap::TYPE_PHPNAME, $indexType)]));
    }
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(CommentTableMap::COL_NEWS_ID);
     ...
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            ...
            foreach ($values as $value) {
                $criterion = $criteria->getNewCriterion(CommentTableMap::COL_NEWS_ID, $value[0]);
                $criterion->addAnd($criteria->getNewCriterion(CommentTableMap::COL_COMMENT_ID, $value[1]));
